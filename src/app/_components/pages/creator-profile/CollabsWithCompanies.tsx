...
export default function CollabsWithCompanies() {
  return (
    <Card className="rounded-[20px] shadow-md p-6 bg-white">
      {/* Header Section */}
      <div className="flex justify-between items-center">
        <h2 className="text-[20px] font-medium text-text">Total collabs with companies</h2>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="text-gray-700 px-3 py-1 rounded-lg">
              Last 7 Days
            </Button>
          </DropdownMenuTrigger>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-success bg-success-light px-2 py-0.5 rounded-md">+3.4%</span>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <StatCard name="Others" value="1,5K" growth="-1.4%" color="blue" isNegative />
      </div>
    </Card>
  );
}
...
function StatCard({ name, value, growth, color, isNegative }: { name: string; value: string; growth: string; color: string; isNegative?: boolean }) {
  return (
    <div className="p-4 rounded-lg border bg-white shadow-sm flex flex-col gap-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xs font-medium px-2 py-0.5 rounded-md ${isNegative ? 'text-red-600 bg-red-100' : 'text-success bg-success-light'}`}>
          {growth}
        </span>
      </div>
    </div>
