  ...
  const fetchCollabWithCompanies = async (days: string) => {
    ...
    try {
      const response = await axios.get(
        `/auth/creator/stats?creatorId=${id}&days=${days}`
      );
      ...
      {loading ? (
        ...
        <div className="bg-blue h-full w-[25%] rounded-r-full"></div>
          ...
          <div className="mt-3 md:mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-3">
            ...
            />
            <StatCard
              name={translate("Conversion_Rate")}
              value={formatNumber(collabStats?.conversionRate)}
              growth=""
              color="secondary-color"
            />
            <StatCard
              name={translate("Orders")}
              value={formatNumber(collabStats?.totalOrders)}
              growth=""
              color="orange"
            />
            <StatCard
            ...
            />
            <StatCard
              name={translate("NumberOFClick")}
              value={formatNumber(collabStats?.visits)}
              growth=""
              color="primary-color"
              isNegative
            />
              ...
              value={
                <div className="flex items-center gap-2">
                  ...
                  <span>{`${formatFloatValue(
                    collabStats?.rating?.totalRating
                  )}/5`}</span>
                </div>
              }
              growth=""
