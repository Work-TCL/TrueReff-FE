...
export default function ChatComponent({ productData, collaborationData }: { productData: IProduct, collaborationData: ICollaboration }) {
  ...
  }, [session?.creator?._id, session?.vendor?._id,isChatView]);
        ...
        {!isLoading &&
          messages.map((msg: any, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`flex items-end gap-2 ${owner ? "justify-start" : "justify-end"
                  }`}
              >
                <div className="flex items-end">
                  ...
                  <div
                    className={`flex flex-col ${owner ? "items-start" : "items-end"
                      } `}
                  >
                    <div
                      className={`p-3 rounded-lg ${owner ? "bg-pink-100" : "bg-gray-100"
                        }`}
                    >
