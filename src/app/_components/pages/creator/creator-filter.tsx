...
const CreatorFilter: React.FC<ICreatorFilterProps> = ({
    ...
    return (
        <>
            ...
            <DialogLayout
                open={openDialog}
                size="!max-w-[638px] w-full overflow-visible m-2 lg:hidden"
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            name="state"
                                ...
                                value={[{value:filterState?.state,label: filterState?.state ? filterState?.state : "Select State"}]}
                                onChange={(val) =>
                                    handleOnSelect(val?.value, 'state',true)
                                }
                                options={[{value:"",label:"Select State"},...indianStates.map((ele) => ({
                                    value: ele,
                                    label: ele,
                                }))]}
                        ...
                        <div>
                            ...
                            name="city"
                                styles={filterCustomStyles}
                                value={[{value:filterState?.city,label: filterState?.city ? filterState?.city : "Select City"}]}
                                onChange={(val) =>
                                    handleOnSelect(val?.value, 'city',true)
                                }
                                options={filterState.state ? [{value:"",label:"Select City"},...cities[filterState.state].map((ele: any) => ({
                                    value: ele,
                                    label: ele,
                                }))] : []}
                                className="basic-multi-select"
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4 relative z-[999]">
                ...
                name="State"
                    ...
                    value={[{value:filterState?.state,label: filterState?.state ? filterState?.state : "Select State"}]}
                    onChange={(val) =>
                        handleOnSelect(val?.value, 'state')
                    }
                    options={[{value:"",label:"Select State"},...indianStates.map((ele) => ({
                        value: ele,
                        label: ele,
