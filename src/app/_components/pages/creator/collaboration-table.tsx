...
const CollaborationTable = ({ data, filter, user, fetchCollaboration }: ICreatorTableProps) => {
    ...
    return (
        <div className="">
            <Table className="min-w-full border border-gray-200 overflow-hidden rounded-2xl">
                <TableHeader className="bg-stroke">
                    <TableRow >
                        ...
                        <CustomTableHead className="w-1/6">{translate("Product_Name")}</CustomTableHead>
                        <CustomTableHead className="w-1/6">{translate("Product_Category")}</CustomTableHead>
                        <CustomTableHead className="w-1/6">{translate(`${user}_Name`)}</CustomTableHead>
                        ...
                        <CustomTableHead className="w-1/6">{translate("Status")}</CustomTableHead>
                    ...
                    {data.length > 0 ? (<>
                        {data.map((collaboration: ICollaboration, index: number) => (
                            <TableRow key={index} className="bg-white">
                                ...
                                <CustomTableCell>{collaboration?.productId?.categories}</CustomTableCell>
                                ...
                                <CustomTableCell>
                                    {{
                                        "REQUESTED": <X className="cursor-pointer" onClick={() => setIsOpen(collaboration?._id)} />,
                                        "PENDING": <div className="flex gap-2">
                                            <Eye className="cursor-pointer" onClick={() => handleProductDetail(collaboration?.productId?._id)} />
                                            <MessagesSquare className="cursor-pointer" onClick={() => handleChatView(collaboration?.productId?._id)} />
                                        </div>,
                                        "REJECTED": null
            ...
            {isOpen && <CancelRequest onClose={()=> setIsOpen("")} collaborationId={isOpen} handleCancelRequest={handleRejectRequest}/>}
        </div>
    );
}
