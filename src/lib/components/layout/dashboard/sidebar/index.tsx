...
const NavLink = ({ link = '', isActive = false, Icon = undefined, label = '', hasSubmenu = false, handleToggle = () => { }, expended = false, isChild = false }: any) => {
  ...
  const classNames = isChild ? childLinkClasses : `flex ${hasSubmenu ? "justify-between":''} items-center text-[16px] cursor-pointer px-4 py-3 space-x-3 rounded-md text-nowrap  ${isActive
    ? hasSubmenu ? 'bg-[#FF49791A] text-black': 'bg-primary-color text-white' 
    : 'text-font-grey hover:bg-pink-100'
    }`
  const iconClassNames = `w-5 h-5 ${isActive ? hasSubmenu ? 'text-black': 'text-white' : 'text-font-grey'
    }`
  if (hasSubmenu) {
    return (
...
const Sidebar = ({expanded,handleExpandSidebar}:ISidebarProps) => {
  ...
  const toggleMenu = (label: string,isSubMenu: boolean) => {
    let keys = Object.keys(expandedMenus).filter(key => expandedMenus[key] === true);
...
setExpandedMenus(obj);
  ...
  return (
    <>
    <aside id="sidebar-multi-level-sidebar" className={`max-w-[300px] w-full h-screen bg-white flex flex-col  fixed lg:relative top-0 left-0 z-40 transition-transform ${expanded ? "-translate-x-full":""} lg:translate-x-0`}>
      <div className="flex justify-end lg:justify-center gap-10">
        ...
        <GoSidebarExpand size={25} className="cursor-pointer float-right mt-5 mr-2 lg:hidden" onClick={handleExpandSidebar}/>
      </div>
